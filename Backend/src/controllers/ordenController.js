...
export const getOrdenes = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === 'cliente') {
      ordenes = await ordenModel.getOrdenesByUsuarioId(req.user.id);
    } else {
      ordenes = await ordenModel.getAllOrdenes();
    }
    res.json(ordenes);
  } catch (error) {
...
export const updateOrden = async (req, res) => {
  try {
    const { id } = req.params;
    const { moto_id, descripcion, fecha_ingreso, fecha_entrega, estado, costo, latitud, longitud } = req.body;

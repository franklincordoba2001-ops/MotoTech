...
export const getClienteByUsuarioId = async (usuarioId) => {
    try {
        const [rows] = await db.query('SELECT * FROM clientes WHERE cliente_id = ?', [usuarioId]);
        return rows[0]; 
    } catch (error) {
        throw new Error('Error al buscar el cliente por usuario: ' + error.message);
    }
};
